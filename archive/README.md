...
- `openai_power_point_estimate.ipynb` — original point-estimate OpenAI
  power→fleet model (renamed from `openai_power_model.ipynb` when the live
  Monte Carlo notebook took that name). Superseded by
  `../notebooks/openai_power_model.ipynb`, which reproduces it exactly at
  `new_chip_share = 0`, zero lag, and nominal power.
