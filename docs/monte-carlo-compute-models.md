...
Two fleet constructions are built from the mix and blended: **default** (vintage-layered — each year's additions keep their vintage mix and carry forward) and **newest** (whole fleet on the latest year's mix). Conversion constants from `data/IT power by chip.csv` and Microsoft's fleet snapshot. **Watts/GPU = server power per GPU × an IT overhead factor** (networking, storage, management); the overhead is now sampled (see the parameter table), giving median watts/GPU A100 926, H100/H200 1,453, GB200 2,090, GB300 2,217. The H100 figure is server-power × overhead for consistency (was a bespoke 1,389 W from SemiAnalysis). Server ratings and the overhead trace to the Colossus data-center calculations sheet (https://docs.google.com/spreadsheets/d/1aXs4vx7AMrzUIR3YeBvJRAD_rmDiioY9zR95DZdDa_s/edit?gid=386717209#gid=386717209), ultimately from [Epoch AI's AI data centers](https://epoch.ai/data/ai-data-centers). H100e/GPU stays fixed: A100 0.315, H100/H200 1.0, B200/B300 2.527.
...
| Power definition factor | mixture: 80% IT `sq.to(0.95, 1.05)` + 20% gross `1 / sq.to(1.10, 1.70)`; one draw shared across all years | Is each disclosed figure IT power or gross (facility) power? 80% it's already IT (a tight residual band around 1.0); 20% it's gross, so true IT is lower by a 1.1–1.7 datacenter PUE. One-sided — it can only pull the total down (median factor ≈0.99, with a downward tail when gross). Shared because a misreading would bias every year the same way. |
...
| IT overhead factor | `sq.to(0.963, 1.349)` clipped at 1.0 (Colossus 10th/median/90th = 1.0 / 1.14 / 1.3, floor 1.0); one draw shared across chips | Watts/GPU = server power × this overhead (networking, storage, management). A **lower** overhead means fewer watts per GPU, so **more** chips per disclosed MW → higher H100e; it moves the total both ways and is the model's only real upside lever. Lives in `lab_model_params.csv` as a shared `chip_specs` prior; sourced from the Colossus sheet, from Epoch AI's AI data centers. |

**Result:** 1.15M / 1.71M / 2.06M H100e end-2025 (main-model deterministic reference: 1.85M). Implied end-2025 count mix (notebook): A100 ~8%, Blackwell ~42% of chips (shares are unchanged — a shared overhead cancels in the mix).

**Uncertainty decomposition (notebook):** two sources now dominate. The **power definition factor** is the largest (90% width ~680k) and one-sided down (the 20% gross tail). The **IT overhead factor** is second (~530k, 1.49M–2.02M) and roughly two-sided — a low overhead buys more chips per MW, so it supplies almost all of the model's *upside*; it is the only source whose 95th (~2.02M) clears the deterministic reference (1.85M), which is why the combined 95th finally rises to 2.06M. `new_chip_share` (~110k) and lag (~210k) mostly move composition, not the total. Sweeping the gross probability (§8) slides the 2025 median from 1.77M at 0% gross to 1.35M at 80% gross; the facility-vs-IT risk is modeled explicitly and lands on the downside, while the IT-overhead lever restores a genuine right tail.
